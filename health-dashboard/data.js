...
/* === Python Human Detection Data Source ===
   Polls the Python YOLO service for human presence.
   Falls back to face-api.js if Python service unavailable. */
async function createPythonDetectionDataSource(pythonApiUrl = "http://localhost:8001") {
  const baseSource = createSimulatedDataSource();
  let pythonAvailable = false;
  let pollTimer = null;
  let lastKnownPresence = false;
  ...
  async function pollHumanPresence() {
    try {
      const res = await fetch(`${pythonApiUrl}/api/human/present`, { 
        method: "GET", 
        signal: AbortSignal.timeout(2000) 
      });
      if (res.ok) {
        ...
        /* Only emit if presence changed */
        if (present !== lastKnownPresence) {
          lastKnownPresence = present;
          baseSource.setPresence(present);
        }
      } else {
  ...
  const originalStart = baseSource.start;
  const originalStop = baseSource.stop;

  return {
    async start(onUpdate) {
      /* First check if Python service is available */
      pythonAvailable = await checkPythonService();
      
      if (pythonAvailable) {
        console.log("[DashboardData] Python human detection service connected");
        /* Start polling for human presence */
        await pollHumanPresence();
        pollTimer = setInterval(pollHumanPresence, 1000);
      } else {
        console.log("[DashboardData] Python service unavailable, using face-api.js");
      }
      
      return originalStart(onUpdate);
    },
    stop() {
      if (pollTimer) {
    ...
    isPythonAvailable() { return pythonAvailable; },
    setPresence: baseSource.setPresence,
    setStudent: baseSource.setStudent,
    pushStripMeasurement: baseSource.pushStripMeasurement,
    sendCommand: baseSource.sendCommand,
    getPresence: baseSource.getPresence,
  };
}
...
window.DashboardData = {
  ...
  createPythonDetectionDataSource,
  PH_ZONES,
  phZone,
};
